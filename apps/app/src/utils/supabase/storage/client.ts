...
export const uploadPrivateFile = async ({
  ...
  if (error) {
    return { fileUrl: '', error: 'Falha no upload do arquivo' };
  }
...
export const uploadPublicImage = async ({
  ...
  if (!PUBLIC_BUCKET) {
    return { imageUrl: '', error: 'Bucket de armazenamento não está definido' };
  }
  ...
  try {
    ...
    console.error(error);
    return { imageUrl: '', error: 'Falha na compressão da imagem' };
  }
  ...
  if (error) {
    return { imageUrl: '', error: 'Falha no upload da imagem' };
  }
