...
export async function sendEmail({ to, subject, html, text }: SendEmailParams) {
  try {
    ...
    let fromEmail = process.env.RESEND_FROM_EMAIL || 'Cuidly Babás <onboarding@resend.dev>';
    // Remove quotes if present (sometimes env vars include them)
    ...
    if (error) {
      console.error('[sendEmail] Erro ao enviar email via Resend:', JSON.stringify(error, null, 2));

      // Tratamento especial para erro de validação do Resend
      const errorMessage = error.message || '';
      if (error.name === 'validation_error' || errorMessage.includes('testing emails') || errorMessage.includes('can only send')) {
      ...
      return { success: false, error: errorMessage || 'Erro desconhecido ao enviar email' };
    }
