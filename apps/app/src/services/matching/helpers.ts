...
import type { NannyProfile, JobData, FamilyData, ChildData } from './index';
...
export function toFamilyData(family: PrismaFamily): FamilyData {
  return {
    ...
    availabilitySlots: buildAvailabilitySlots(family.neededDays, family.neededShifts),
...
/**
 * Convert Prisma Child to ChildData for matching
 */
export function toChildData(child: PrismaChild): ChildData {
  return {
    id: child.id,
...
function buildAvailabilitySlots(
  ...
  if (!neededDays || neededDays.length === 0 || !neededShifts || neededShifts.length === 0) {
    return null;
  }
...
function parseAvailabilitySlots(availabilityJson: unknown): string[] | null {
  ...
  const availability = availabilityJson as Record<string, { enabled?: boolean; startTime?: string; endTime?: string }>;
