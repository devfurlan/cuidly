...
import { detectContactInformation } from './contact-detection';
...
const RED_FLAGS = [
  // Medical promises
  'curo', 'garanto cura', 'recuperação garantida', 'tratamento médico',

  // Too informal
  'tipo assim', 'né', 'kkk', 'rsrs', 'haha',

  // Negative language
  'não tenho experiência', 'sem experiência', 'nunca fiz', 'não sei',

  // Protected medical terms (without proper qualification)
  'diagnóstico', 'prescrevo', 'receito', 'cirurgia',
];
...
export function generateCacheKey(data: any): string {
  ...
  const keyData = {
    ...
    serviceTypes: Array.isArray(data.serviceTypes) ? data.serviceTypes.sort() : [],
