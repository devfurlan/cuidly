...
>(
  (
    ...
    // Sync internal state with external date prop changes
    React.useEffect(() => {
      if (date?.from) {
        setMonthFrom(date.from);
        setYearFrom(date.from.getFullYear());
      }
      if (numberOfMonths === 2 && date?.to) {
        setMonthTo(date.to);
        setYearTo(date.to.getFullYear());
      } else if (numberOfMonths === 1 && date?.from) {
        setMonthTo(date.from);
        setYearTo(date.from.getFullYear());
      }
    }, [date, numberOfMonths]);
