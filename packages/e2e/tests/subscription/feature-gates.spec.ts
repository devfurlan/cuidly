...
test.describe("Subscription: Feature Gates", () => {
  test.describe("Family feature gates", () => {
    test("free family should see upgrade modal on subscription page", async ({
      context,
      page,
    }) => {
      ...
      await expect(page.getByText(/Assinar Plus/i).first()).toBeVisible({
        timeout: 10000,
      });
    });

    test("paid family should NOT see upgrade button on subscription page", async ({
      context,
      page,
    }) => {
    ...
    test("free family should see payment history empty", async ({
      context,
      page,
    }) => {
      await loginAsFamily(context, page);
      ...
      await expect(page.getByText("Histórico de Pagamentos")).not.toBeVisible();
    });

    test("paid family should see payment history", async ({
      context,
      page,
    }) => {
      await loginAsFamilyPaid(context, page);
      ...
      await expect(page.getByText("R$\u00a047,00").first()).toBeVisible();
  ...
  test.describe("Nanny feature gates", () => {
    ...
    test("free nanny clicking upgrade should open NannyProUpsellModal", async ({
      context,
      page,
    }) => {
      await loginAsNanny(context, page);
    ...
    test("pro nanny should NOT see upgrade button", async ({
      context,
      page,
    }) => {
  ...
  test.describe("Subscription status indicators", () => {
    ...
    test("should show free plan label correctly", async ({ context, page }) => {
      ...
      await expect(page.getByText(/Gratuito|Free/i).first()).toBeVisible({
        timeout: 15000,
      });

      // Should see "Sem cobrança"
      await expect(page.getByText(/Sem cobrança/i)).toBeVisible();
    });
  });
});
