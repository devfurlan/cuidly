...
test.describe("Family: Search Nannies", () => {
  ...
  test("should display nanny count after loading", async ({ page }) => {
    ...
    await expect(page.getByText(/\d+ babás? disponíve/i)).toBeVisible({
      timeout: 30000,
    });
  });
  ...
  test("should show filter options when filters are expanded", async ({
    page,
  }) => {
    await page.goto("/app/babas");
  ...
  test("should show matching info for paid family with active job", async ({
    page,
  }) => {
    await page.goto("/app/babas");

    // familyPaid has an active job, so matching info should appear after load
    await expect(page.getByText(/compatibilidade/).first()).toBeVisible({
      timeout: 30000,
    });
  });
});
